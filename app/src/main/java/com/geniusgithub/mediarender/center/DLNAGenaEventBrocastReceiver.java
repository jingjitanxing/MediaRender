...
import com.gxh.dlnalib.PlatinumJniProxy;
		...
		switch(cmd){
			...
			case PlatinumReflection.MEDIA_RENDER_TOCONTRPOINT_SET_MEDIA_PLAYINGSTATE:
				...
				if (playingstate.equalsIgnoreCase(PlatinumReflection.MEDIA_PLAYINGSTATE_STOP)){
					String dur = intent.getStringExtra(PlatinumReflection.GET_PARAM_MEDIA_DURATION);
					log.i("set string duration is " + dur);
					PlatinumJniProxy.responseGenaEvent(PlatinumReflection.MEDIA_RENDER_TOCONTRPOINT_SET_MEDIA_POSITION, dur, null);
				}
