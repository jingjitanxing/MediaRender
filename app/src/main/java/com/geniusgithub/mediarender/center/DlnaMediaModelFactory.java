...
public class DlnaMediaModelFactory {
	...
	public static final String PARAM_PLAYER_COUNT = "param_player_count";
	
	static long playingCount = 0;
	public static void pushMediaModelToIntent(Intent intent, DlnaMediaModel mediaInfo){
		...
		intent.putExtra(PARAM_PLAYER_COUNT, mediaInfo.getPlayingCount());
	}
	...
	public static DlnaMediaModel createFromIntent(Intent intent){
		...
		mediaInfo.setPlayingCount(intent.getLongExtra(PARAM_PLAYER_COUNT, 0));
		return mediaInfo;
	}
	...
	public static DlnaMediaModel createFromMetaData(String metadata){
		...
		try {
			...
			mediainfo.setPlayingCount(playingCount);
			playingCount++;
		} catch (Exception e) {
			e.printStackTrace();
		}
