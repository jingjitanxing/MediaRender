...
import static com.gxh.video_cache.Preconditions.checkArgument;
...
class Pinger {
    ...
    boolean ping(int maxAttempts, int startTimeout) {
        ...
        while (attempts < maxAttempts) {
            try {
                ...
                Log.i(TAG,"Error pinging server (attempt: " + attempts + ", timeout: " + timeout + "). ");
            } catch (InterruptedException | ExecutionException e) {
                Log.i(TAG,"Error pinging server due to unexpected error", e);
            }
            attempts++;
            timeout *= 2;
        }
        ...
        Log.i(TAG,error, new ProxyCacheException(error));
        return false;
    }
    ...
    private boolean pingServer() throws ProxyCacheException {
        ...
        try {
            ...
            Log.i(TAG,"Error reading ping response", e);
            return false;
        } finally {
            source.close();
        }
